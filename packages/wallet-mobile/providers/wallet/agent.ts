 ...
 * Added debug logging and enhanced error handling for React Native
 *
...
export function createVeramoAgent(options: AgentOptions): AppAgent {
  console.log('[createVeramoAgent] Creating Veramo agent...')
        ...
        resolver: new Resolver({
          ...
          ...webDidResolver(async (did: string, url: string) => {
            ...
            if (did.startsWith('did:web:localhost')) {
              try {
                console.debug('[createVeramoAgent] Resolving local DID:', did)
                ...
                console.error('[createVeramoAgent] Error fetching local DID document:', e)
              }
            }
            try {
              console.debug('[createVeramoAgent] Attempting to fetch DID from:', url)
              const res = await fetch(url, { mode: 'cors' })
              if (res.status >= 400) {
                console.warn('[createVeramoAgent] DID fetch returned status:', res.status)
                return null
              }
              return res.json()
            } catch (e) {
              console.warn(
                '[createVeramoAgent] CORS or fetch error for DID resolution (may be expected in mobile):',
                e
              )
              return null
            }
  ...
  console.log('[createVeramoAgent] ✓ Veramo agent created successfully')
  return agent
}
